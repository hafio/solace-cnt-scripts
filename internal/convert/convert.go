...
//
// It decodes STRICTLY, the way config.Load does. A plain yaml.Unmarshal accepts keys
// the real loader rejects, so a converted file carrying one would pass here and fail
// at the operator's next command -- which is the opposite of what this check is for.
func validateOutput(body string, p config.Platform) error {
	c, err := config.DecodeStrict([]byte(body))
	if err != nil {
...
func emitYAML(v *vars, p config.Platform, source string) (string, []string) {
	...
	// document structure, so strip control characters first.
		...
		d.block("additionalUsers", func(d *doc) {
			for i, entry := range v.l("SOLBK_USR_PASS") {
				...
				if !ok || user == "" {
					...
					// so the text itself is the secret: no value in a log line.
...
func redundancy(v *vars, p config.Platform) (string, []string) {
	...
	// boolOf owns the true/false spellings the bootstraps used, so this cannot come to
	// accept a word the rest of the converter does not.
	if b, ok := boolOf(raw); ok {
...
func (d *doc) section(key string, fill func(*doc)) { d.topBlock(key, fill, false) }
...
func (d *doc) sectionMarker(key string, fill func(*doc)) { d.topBlock(key, fill, true) }

// topBlock writes one top-level section; block, just above, is the nested-key writer.
// keepEmpty is the whole difference between the two callers above: an ordinary section
// with nothing in it is omitted, while the target platform's own section is written as
// `key: {}`.
func (d *doc) topBlock(key string, fill func(*doc), keepEmpty bool) {
	sub := &doc{indent: d.indent + 1}
	fill(sub)
	if sub.b.Len() == 0 && !keepEmpty {
		return
	}
...
// scalarEscaper escapes what a YAML double-quoted scalar cannot carry raw: the quote
// and backslash, and EVERY control character -- not only the three with a short
// spelling. A bash file can hold an ANSI escape inside a password, and passed through
// unescaped it is invalid YAML at best and a terminal control sequence at worst.
var scalarEscaper = func() *strings.Replacer {
	pairs := []string{`\`, `\\`, `"`, `\"`, "\n", `\n`, "\t", `\t`, "\r", `\r`}
	for r := rune(0); r < 0x20; r++ {
		if r == '\n' || r == '\t' || r == '\r' {
			continue
		}
		pairs = append(pairs, string(r), fmt.Sprintf(`\x%02x`, r))
	}
	pairs = append(pairs, "\x7f", `\x7f`)
