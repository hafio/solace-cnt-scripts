...
const (
	...
	// userUnitDropIn caps a ROOTLESS podman: its engine runs under
	// user@<uid>.service with no CAP_SYS_RESOURCE, so that unit's hard limits are
	// the most the container can be given.
	userUnitDropIn = "/etc/systemd/system/user@.service.d/99-solace.conf"
)
...
func (m *Manager) checkLimits(ctx context.Context, fix bool) error {
	...
	return errors.Join(m.checkNrOpen(ctx, fix), m.checkUserManagerLimits(ctx))
}
...
func (m *Manager) checkNrOpen(ctx context.Context, fix bool) error {
	...
	return fmt.Errorf("fs.nr_open is %d, below the %d the broker needs.\n%s",
...
func (m *Manager) nrOpenRemedy() string {
	return fmt.Sprintf("  sudo sysctl -w fs.nr_open=%d\n"+
		"  echo 'fs.nr_open = %d' | sudo tee %s",
...
func (m *Manager) checkUserManagerLimits(ctx context.Context) error {
	...
	return fmt.Errorf("rootless podman cannot raise its own limits, and %s: %s.\n"+
		"  An administrator must create %s:\n"+
		"    [Service]\n"+
		"    LimitNOFILE=%s\n"+
		"    LimitMEMLOCK=%s\n"+
		"    LimitCORE=%s\n"+
		...
		unit, strings.Join(short, "; "), userUnitDropIn,
...
func parseUnitProps(out []byte) map[string]int {
	...
	for _, line := range strings.Split(string(out), "\n") {
		...
		if n, err := parseLimit(v, "systemctl show"); err == nil {
			props[k] = n
		}
	}
	return props
