...
**Removals keep the expensive layer (`helpers.go`: `layer`, `addRemoveFlags`, `confirmLayer`).**
...
(`helpers.go`: `stdinCanAnswer`, `promptLine`). The rule cost two bugs to learn. The
...
`confirmActionStrict` in `internal/cli/helpers.go`), asked ONCE for the whole
