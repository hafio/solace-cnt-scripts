...
| `TestCommandTildeExpansion` | `expandCommandHomes` field by field -- `kubernetes.command`, `docker.command`, `docker.compose`, `podman.command`, `replication.sites[].via.kubernetes.command` -- and every case runs `CheckCommand` afterwards, because expansion and the guard are one feature: the point of expanding is that `--kubeconfig ~/solace/kubecontext` then passes, so a token that expanded to something the guard refuses fails here. It is the drift test between the pass' field list and `validateExecCommands`' -- a field missing from the pass keeps its literal `~` and its row fails. `docker.compose`'s row uses the STANDALONE `docker-compose` binary because the guard leaves no other option: `checkFlagShape` admits the `compose` subword only as the LAST token and only directly after an allowed binary, so `docker --config ~/dk compose` is refused for the subword position rather than for the tilde, and a tilde in that field is only expressible through the standalone form. Four sub-cases carry the rest of the rule: argv[0] is left ALONE and then refused by `checkBinary` (a home directory cannot be part of a bare binary name), an 8.3 short name survives untouched, an unresolvable home fails naming the field and the underlying cause rather than letting a literal `~` reach the guard, and a home containing a space is refused HERE -- `checkToken`'s own whitespace message tells the operator to split the argument into list entries, which is wrong advice for a space they never typed |
...
| `TestValidateContainerMonitorCPUSet` | The monitor takes exactly one cpu whatever the tier: single ids and the unset case pass, a multi-cpu set is refused naming the key and the one-cpu rule (the tier's set belongs under `container.cpuset`), and malformed values are refused as they are for `cpuset` |
...
| `TestCheckHostPathAccepts` | The shapes an operator legitimately writes, including the two Windows forms a command token may not carry and an EMBEDDED tilde (`C:\Users\RUNNER~1\certs`, an 8.3 short name). A LEADING one is deliberately NOT here -- it sits in the rejects table, because `expandHomePaths` runs over every host-path key before `Validate` reaches this gate, so a `~` still arriving is a wiring failure rather than a shape anyone writes |
| `TestCheckHostPathRejects` | Each refusal with its own reason, so a message that stops naming the cause fails here rather than confusing an operator -- and that includes all four leading-`~` shapes (`~`, `~/...`, `~\...`, `~user/...`), whose message must say the Config was not loaded from a file rather than that a tilde is disallowed in a path. That is the fail-closed half of the expansion feature: the alternative to failing is a literal `~` reaching a quadlet `Volume=` line, a compose mount source, or the `rm -rf` behind `broker remove --delete-data` |
...
| `TestLoadExpandsATildeThroughTheWholePipeline` | The WIRING of BOTH passes, which nothing else pinned: every other tilde test calls `expandTilde`, `expandHomePaths` or `expandCommandHomes` directly, so deleting either call from `Load` left the suite green. That is a real gap rather than a stylistic one, because the behaviour it replaced -- an outright refusal of a leading `~` -- was reached through `Validate`, whose own wiring IS pinned, so the swap traded a tested behaviour for an untested one. Drives the real home-directory lookup through a full `Load`, and asserts both that the tilde is gone and that the result sits under the home directory rather than being rebased onto the env file's own directory, which is exactly the failure the old refusal existed to prevent. It carries `kubernetes.command: kubectl --kubeconfig ~/kc` for the command half -- the exact line whose refusal reported this feature missing -- so one `Load` proves the token expanded AND that the guard `Validate` runs next accepted the expanded token, which is also why `tildeHome` normalises separators to `/`: `\` is in `unsafeTokenChars`, so a Windows home spliced in verbatim would be refused by the check the pass exists to satisfy |
| `TestExpandHomePathsCoversEveryFieldItClaims` | Every field the pass lists, set at once, because the failure this shape is most likely to have is a MISSING expansion rather than a wrong one: `expandHomePaths` is a hand-written list, so a field added to the schema later -- or dropped from the list in a refactor -- reaches the filesystem with a literal `~` in it and surfaces as a missing file far from the env file. Setting two CAs, two certificate dirs and two map entries also pins the three loops, which a one-value-per-field test cannot. The list now includes the four container dir keys (`docker.container.dataDir`, `podman.quadletDir`, `podman.baseDir`, `podman.container.dataDir`), and they are set UNCONDITIONALLY rather than per platform, mirroring the pass itself: a per-platform list there would be a second enumeration of the schema, and a per-platform loop here would not notice one appearing |
...
| `TestMonitorIsSizedForQuorumNotForTheTier` | The monitor arbitrates quorum, carries no spool and routes no traffic, so it takes a fixed 1 cpu and 2g instead of the tier. One fixture rendered at both roles, so the difference is attributable to the role alone; the lines that must NOT differ are asserted too -- shm and the ulimits are broker requirements, not sizing -- and so is the compose half |
| `TestMonitorCPUSetOverrideReachesArtifact` | Which cpu the monitor runs on is the operator's, the same split `container.cpuset` has: the count is what stays fixed |
